...
st.set_page_config(page_title="전국 도서관 표준데이터 분석", layout="wide")

CSV_PATH = "전국도서관표준데이터.csv"

# 데이터 시각화 팔레트 (고정 순서 — 항목별 색상 일관성 유지)
...
def load_data():
    ...
    df["시도명"] = df["시도명"].replace({"전라북도": "전북특별자치도"})
...
st.sidebar.title("📚 전국 도서관 표준데이터")
page = st.sidebar.radio(
    "메뉴",
    ["개요", "지역별 분석", "유형별 분석", "운영시간 분석", "지도", "데이터 탐색"],
)

st.sidebar.markdown("---")
st.sidebar.subheader("필터")
sel_sido = st.sidebar.multiselect("시도", sorted(df["시도명"].unique()))
sel_type = st.sidebar.multiselect("도서관유형", type_order)

fdf = df.copy()
if sel_sido:
    fdf = fdf[fdf["시도명"].isin(sel_sido)]
if sel_type:
    fdf = fdf[fdf["도서관유형"].isin(sel_type)]
st.sidebar.caption(f"필터 적용 결과: {len(fdf):,}개관 / 전체 {len(df):,}개관")


# ---------------------------------------------------------------- 개요
if page == "개요":
    st.title("전국 공공도서관 현황 개요")
    ...
    with col1:
        st.subheader("시도별 도서관 수")
        by_sido = fdf["시도명"].value_counts().reset_index()
        by_sido.columns = ["시도명", "도서관수"]
        fig = px.bar(by_sido, x="도서관수", y="시도명", orientation="h",
...
# ---------------------------------------------------------------- 지역별 분석
    ...
    if metric == "도서관 수":
        agg = fdf.groupby("시도명").size().reset_index(name="값")
    else:
        agg = fdf.groupby("시도명")[metric].sum().reset_index(name="값")
    agg = agg.sort_values("값", ascending=False)

    fig = px.bar(agg, x="시도명", y="값", color="값",
                 color_continuous_scale=SEQUENTIAL_BLUE)
    fig.update_layout(xaxis={"categoryorder": "total descending"},
                       coloraxis_showscale=False)
    st.plotly_chart(fig, use_container_width=True)

    st.subheader("시도별 도서관 유형 구성")
    stack = fdf.groupby(["시도명", "도서관유형"]).size().reset_index(name="도서관수")
    sido_order = fdf["시도명"].value_counts().index.tolist()
    fig2 = px.bar(stack, x="시도명", y="도서관수", color="도서관유형",
...
# ---------------------------------------------------------------- 운영시간 분석
    ...
    )[["도서관명", "시도명", "시군구명", "도서관유형", "평일운영시작시각", "평일운영종료시각", "평일운영시간"]].head(20)
...
# ---------------------------------------------------------------- 지도
    ...
    fig = px.scatter_map(
        ...
        color_discrete_map=TYPE_COLOR,
        hover_name="도서관명",
        hover_data={"시도명": True, "소재지도로명주소": True, "위도": False, "경도": False},
        zoom=6, height=650,
    )
...
# ---------------------------------------------------------------- 데이터 탐색
    ...
    st.download_button("CSV 다운로드", data=csv, file_name="도서관_검색결과.csv", mime="text/csv")
